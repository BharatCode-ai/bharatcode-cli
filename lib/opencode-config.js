...
function spawnCommand(command, args, { stdio = "inherit" } = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code, signal) => {
      if (signal) reject(new Error(`BharatCode engine plugin installer exited with signal ${signal}`))
      else if (code) reject(new Error(`BharatCode engine plugin installer failed with exit code ${code}`))
      else resolve()
    })
  })
}
